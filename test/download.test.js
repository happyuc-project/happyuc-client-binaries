...
test['hash sha256 mismatch'] = function* () {
  ...
  try {
    ...
    err.message.should.contain(
      `Hash mismatch (using sha256): expected blahblahblah; got e7781ccd95e2db9246dbe8c1deaf9238ab4428a713d08080689834fd68a25652`);
  }
};
...
test['hash md5 mismatch'] = function* () {
  ...
  try {
    ...
    err.message.should.contain(
      `Hash mismatch (using md5): expected blahblahblah; got dff641865ffb9b44d53f1f9def74f2e6`);
  }
};
...
test['unpacks and verifies ok'] = function* () {
  ...
  _get(ret, 'downloadFile', '').
    should.
    eql(path.join(downloadFolder, `archive.zip`));
  _get(ret, 'unpackFolder', '').
    should.
    eql(path.join(downloadFolder, `unpacked`));

  _get(ret, 'client.state.available', '').should.be.true;
  _get(ret, 'client.activeCli.fullPath', '').
    should.
    eql(path.join(downloadFolder, `unpacked`, 'maga2'));
...
test['unpacked updated version and copied over old version'] = function* () {
  ...
  let buildMgrOpts = function(scope, downloadOpts) {
    return {
      clients: {
        'Maga2': {
          'platforms': scope.buildPlatformConfig(process.platform, process.arch,
            downloadOpts),
        },
      },
    };
  ...
  _get(ret, 'client.activeCli.fullPath', '').
    should.
    eql(path.join(downloadFolder, 'unpacked', 'maga3'));
  ...
  let ret2 = yield mgr2.download('Maga2',
    {downloadFolder: path.join(downloadFolder, '..')});

  _get(ret2, 'client.activeCli.fullPath', '').
    should.
    eql(path.join(downloadFolder, 'unpacked', 'maga3'));

  // check that maga3 === maga2-special
  const hash1 = md5File.sync(path.join(downloadFolder, 'unpacked', 'maga3'));
  const hash2 = md5File.sync(
    path.join(downloadFolder, 'unpacked', 'maga2-special'));
